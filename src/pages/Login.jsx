  ...
  const handleLogin = (e) => {
    ...
    setTimeout(() => {
      ...
      Swal.fire({
        ...
        text: "Selamat Datang di Sistem Queen Laundry",
        icon: "success",
        confirmButtonColor: "#1B305B", // Queen Navy
        timer: 2000,
      }).then(() => {
        navigate("/dashboard"); // PINDAH KE DASHBOARD SETELAH SWAL SELESAI
      });
    }, 1500);
  };
      ...
      <div className="max-w-md w-full space-y-8 bg-white dark:bg-slate-800 p-10 rounded-3xl shadow-2xl border border-gray-100 dark:border-slate-700 relative overflow-hidden">
        ...
        <div className="text-center">
          <div className="relative mx-auto h-24 w-24 mb-6 group">
            {/* Efek Glow Emas di belakang logo */}
        ...
        <form className="mt-8 space-y-6" onSubmit={handleLogin}>
          ...
          <button
            type="submit"
            disabled={loading}
            className={`group relative w-full flex justify-center py-3.5 px-4 border border-transparent text-sm font-bold rounded-xl text-white bg-queen-navy dark:bg-queen-navy hover:bg-opacity-90 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-queen-navy transition-all shadow-xl active:scale-95 ${loading ? "opacity-70 cursor-not-allowed" : ""}`}
          >
